...
import UISearchableSelect from './UISearchableSelect.jsx';
import UIAutocompleteAPI from './UIAutocompleteAPI.jsx';
...
const ItemCompraRow = ({ item, onItemChange, onItemRemove, categoryOptions }) => {
    ...
    return (
        <div className="fc-rateio-linha" style={{ gridTemplateColumns: 'minmax(0, 2.5fr) 90px 110px 110px minmax(0, 2fr) 40px' }}>
            ...
            <UISearchableSelect options={categoryOptions} placeholder="Categoria..." onChange={(val) => handleChange('id_categoria', val)} initialValue={item.id_categoria} />
...
const ItemRateioRow = ({ item, onItemChange, onItemRemove, categoryOptions }) => {
    ...
    return (
        <div className="fc-rateio-linha" style={{ gridTemplateColumns: '2.5fr 2.5fr 2fr 130px 40px' }}>
            <UIAutocompleteAPI apiEndpoint="/api/financeiro/contatos" placeholder="Buscar funcionário/sócio..." onSelectionChange={(selection) => handleChange('favorecido', selection)} initialSelection={item.favorecido} />
            <UISearchableSelect options={categoryOptions} placeholder="Categoria..." onChange={(val) => handleChange('id_categoria', val)} initialValue={item.id_categoria} />
