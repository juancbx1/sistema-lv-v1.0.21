...
| **Rede de segurança pós-E2/E3** | `api/producao.js GET /status-funcionarios` garante registro em `ponto_diario` mesmo quando supervisor esqueceu de liberar E nenhuma tarefa foi finalizada dentro da tolerância. Usa horários AGENDADOS como fallback. Só aplica a funcionários com evidência de atividade hoje (sessões ou tarefa ativa) e pula `FALTOU`/`ALOCADO_EXTERNO`. |
| **Botão "Liberar para intervalo"** | Aparece no intervalo `[S-25, S+30]` com 4 fases visuais escalonadas: `fase-antecipacao` (azul suave), `fase-iminente` (amarelo pulse lento), `fase-hora-agora` (laranja forte pulse rápido), `fase-atrasado` (vermelho + contador de atraso + pulse na borda do card). Só aparece se o intervalo do dia ainda não foi registrado. Separado do `.cracha-footer` para não comprimir "Atribuir Tarefa". Após `S+30`, a rede de segurança do backend assume e o botão some. |
